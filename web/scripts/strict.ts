 ...
 *
 *   npx tsx scripts/strict.ts --tf 15m,1h,4h --bars 3000
 *
...
async function main() {
  ...
  console.log('Thứ tự trong nến: giả định thận trọng (nhánh này chưa có nến 1m)\n');
  ...
  for (const tf of tfs) {
    for (const symbol of symbols) {
      ...
      const a = runBacktestStrict(symbol, tf, candles, DEFAULT_BT, { rr1Penalty: true });
      const b = runBacktestStrict(symbol, tf, candles, DEFAULT_BT, { rr1Penalty: false });
