...
## 9b. Thước mới — kết quả chạy lại backtest
...
Vì cả ba thay đổi này đổi cả tín hiệu lẫn mức giá, **toàn bộ avgR / PF / sụt giảm
ở mục 10 là số đo trên hệ CŨ và không mô tả hệ hiện tại.**

Backtest đã được chạy lại trên thước mới (`lib/backtestNew.ts`,
`npm run backtest:new`). Đường chạy mới dùng **đúng `readTF()` mà API đang chạy**,
và **đúng hàm `simulate()`** của backtest cũ, nên hai bộ số so được với nhau. Nến
khung lớn được gộp từ 15m thay vì tải riêng, để mốc đóng không bao giờ lệch.

### Kết quả: KHÔNG CÓ LỆNH NÀO

BTC / ETH / SOL / ENA · khung 1h và 4h · 5.999 nến 15m mỗi mã (05/07 → 05/09/2026)
· 6.232 nến được xét:

| Sàn điểm | Số lệnh | Ghi chú |
|---|---|---|
| **7 (đang chạy thật)** | **0** | điểm cao nhất đo được trên toàn mẫu là **6.5** |
| 6.5 (chẩn đoán) | 0 | các cửa khác vẫn chặn |
| 5.5 (chẩn đoán) | 0 | như trên |
| 4.5 (chẩn đoán) | 26 | avgR −0.54 · PF 0.14 · cả hai nửa đều âm |

### Vì sao 0 lệnh: đây là trần SỐ HỌC, không phải kết luận về thị trường

Backtest chạy **mù phái sinh** (fapi chặn IP nên OI / funding / taker perp là
N/A). Hai vế cộng điểm phụ thuộc phái sinh vì thế **không bao giờ bắn được**:

| Vế | Điểm | Bắn được trong backtest? |
|---|---|---|
| PA ở mép | +2 | có (44.8% số nến) |
| Giá tại VAL/VAH lớp ngắn | +2 | có (13.8% + 12.8%) |
| Nến đóng xác nhận + vol ≥ median 20 | +1.5 | có (31.0%) |
| Delta cùng chiều đúng khung | +1 | có (57.4%) |
| **Spot và hợp đồng cùng nghiêng** | **+1** | **không — cần taker perp** |
| **Funding cùng hướng** | **+0.5** | **không — cần funding** |

Tổng cộng đạt được tối đa: 2 + 2 + 1.5 + 1 = **6.5**, đúng bằng điểm cao nhất đo
được. **Sàn 7 không thể với tới khi thiếu dữ liệu phái sinh.** Đây là cùng một
kiểu lỗi với định nghĩa hạng vàng bản đầu (bắn 0/1831 lần) — điều kiện đặt ra
vượt quá thứ dữ liệu có thể cung cấp.

Khi chạy thật trên Vercel thì phái sinh có thật, trần lên **8.0**. Nhưng sàn 7
khi đó đòi gần như MỌI vế dương cùng bắn và KHÔNG vế âm nào — đo trên bản điện
đang chạy, 12 khung của 3 mã cho điểm cao nhất là 5.5.

### Điều gì chặn sau khi bỏ qua sàn điểm

Ở sàn chẩn đoán 4.5, tỷ lệ nến bị chặn theo từng lý do:

| Lý do | % số nến |
|---|---|
| còn trong vùng | 73.8% |
| khung 4 giờ chỉ lọc, không tự phát lệnh | 20.5% |
| cắt hoặc chốt rơi giữa cụm mỏng | 1.8% |
| không có cụm dày phía trước để chốt | 0.2% |

Luật "còn trong vùng thì không mở lệnh mới" mới là thứ lọc nặng nhất, và nó đang
làm đúng việc của nó.

### Ba điều KHÔNG được suy ra từ bảng trên

1. **Không kết luận được hệ mới tốt hay xấu.** 0 lệnh thì không có gì để đo, và
   26 lệnh ở sàn chẩn đoán 4.5 là mẫu quá nhỏ để nói bất cứ điều gì về kỳ vọng.
2. **Không được hạ sàn để có số đẹp.** Cờ `--floor` chỉ tồn tại để chẩn đoán và
   luôn in cảnh báo; đường chạy thật không đọc nó.
3. **Chưa kiểm chứng được bảng điểm mới.** Muốn kiểm chứng thì backtest phải
   chạy từ nơi vào được `fapi`. Đó là việc hạ tầng, không phải việc thuật toán.

### Cửa sổ và giả định phí

15m 1000 nến (live) / 6000 nến (backtest) · 1h 168 · 4h 126 · 1d 90. Phí 0.05%
mỗi chiều cộng 0.02% trượt giá khi thoát bằng stop — giữ nguyên như bản cũ để hai
bộ số so được với nhau.
